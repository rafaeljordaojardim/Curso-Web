//implementação dos comportamentos vai ser aqui
//criar o novo elemento
function novoElemento(tagName, className) {
    const elem = document.createElement(tagName)
    elem.className = className //usa-se esse ou o classList e adiciona
    return elem
}
...
//altura abertura e a posição x que quer colocar
    ...
    this.animar = () => {
        this.pares.forEach(par => {
            ...
            if (par.getX() < -par.getLargura()) {
                ...
                par.sortearAbertura() //sortear para  outra abertura
            }
...
// const barreiras = new Barreiras(500, 800, 160, 400)
...
function estaoSobrepostos(elementoA, elementoB) {
    //return o retangulo associoado ao elemento A
    const a = elementoA.getBoundingClientRect()
    const b = elementoB.getBoundingClientRect()

    const ladoDireitoA = a.left + a.width;
    const ladoEsquedoB = b.left
    const ladoDireitoB = b.left + b.width
    const ladoEsquerdoA = a.left
    const horizontal = ladoDireitoA >= ladoEsquedoB &&
...
function FlappyBird() {
    ...
    const barreiras = new Barreiras(altura, largura, 160, 400,
